...
const Header = () => {
    ...
    return (
        ...
        >
                ...
                <nav className="hidden md:flex items-center space-x-10">
                    {navItems.map((item) => (
                        <a
                            key={item.name}
                            href={item.href}
                            className="text-white/90 hover:text-orange-primary font-medium transition-colors nav-link"
                        >
                            {item.name}
                        </a>
                    ))}
            ...
            {/* Mobile Nav */}
            {isMobileMenuOpen && (
                <div className="md:hidden glass-effect absolute top-full left-0 right-0 py-6 border-t border-white/10 animate-in fade-in slide-in-from-top-4">
